...
void identify_ata(uint8_t drive) {
    ...
    if (tmp & STATUS_RDY) {
        switch (drive) {
            case 0xA0:
                kprintln("Master Drive is the current active Drive");
                break;
            case 0xB0:
                kprintln("Slave Drive is the current active Drive");
                break;
        }
    }
    else
    {
        switch (drive) {
            case 0xA0:
                kprintln("Master Drive is NOT the current active Drive");
                break;
            case 0xB0:
                kprintln("Slave Drive is NOT the current active Drive");
                break;
            }
    }
}
...
uint8_t ata_disk_status(uint8_t print_disk_status) {
    uint8_t status = inb(ATA_PRIMARY_IO + 7);
    if (status & STATUS_ERR) {
        if(1 == print_disk_status)
            kprintln("Error occurred");
    }
    if (status & STATUS_DF) {
        if(1 == print_disk_status)
            kprintln("Drive fault");
    }
    if (status & STATUS_RDY) {
        if(1 == print_disk_status)
            kprintln("Drive is ready");
        status = 0;
    }
